...
module "redis" {
  ...
  display_name   = "Redis cache for school backend (${var.environment})"
  labels = {
...
module "backend_service" {
  ...
  port                              = 3000
  min_instances                     = 1 # Production: always keep at least 1 instance warm
  max_instances                     = 2
  allow_unauthenticated             = true
  cloudsql_instance_connection_name = module.database.connection_name
  custom_domain                     = var.backend_domain

  # Production: use Memorystore Redis (STANDARD_HA with automatic failover)
  # instead of ephemeral sidecar. Durable cache/session data across pod restarts.
  redis_sidecar_enabled = false
  ...
  secret_env_vars = {
    ...
    # Production secrets: ensure these exist in Secret Manager with prod names.
    # Contact ops/secrets team to create and populate:
    # - school-prod-sendgrid-api-key
    # - school-prod-twilio-auth-token
    # - school-prod-twilio-account-sid
    SENDGRID_API_KEY   = { secret_id = "school-prod-sendgrid-api-key" }
    TWILIO_AUTH_TOKEN  = { secret_id = "school-prod-twilio-auth-token" }
    TWILIO_ACCOUNT_SID = { secret_id = "school-prod-twilio-account-sid" }
  }
}
